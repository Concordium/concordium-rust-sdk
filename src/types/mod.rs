...
pub mod block_certificates {
    ...
    #[derive(Debug)]
    pub struct QuorumSignature(pub [u8; 48]);

    /// A quorum certificate on a block.
    #[derive(Debug)]
    pub struct QuorumCertificate {
        /// `Hash` of the block that this `QuorumCertificate`
        /// certifies.
    ...
    /// by members of the finalization committee.
    #[derive(Debug)]
    pub struct TimeoutSignature(pub [u8; 48]);
    ...
    pub struct BlockCertificates {
        ...
        /// The timeout certificate is present if and only if
        /// the previous round of the block timed out.
        pub timeout_certificate:      Option<TimeoutCertificate>,
        /// The epoch finalization entry is present if and only if
