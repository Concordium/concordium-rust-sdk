...
use concordium_base::{
    ...
    web3id::{self, v1::{PresentationV1, ProofMetadataV1}},
};
...
pub async fn verify_credential_metadata(
    ...
    match &metadata.cred_metadata {
        CredentialMetadata::Account ( metadata, .. ) => {
            let issuer = metadata.issuer;
            let cred_id = metadata.cred_id;
            let ai = client
        ...
        CredentialMetadata::Web3Id ( metadata, .. ) => {
            let contract = metadata.contract;
            let holder = metadata.holder;
...
/// verify credential proofs for identity - v2 for now
pub async fn verify_credential_metadata_v1(
    mut client: v2::Client,
    network: web3id::did::Network, // TODO - currently unusued, not sure if we need it here or not
    metadata: &ProofMetadataV1,
    bi: impl IntoBlockIdentifier,
) -> Result<CredentialWithMetadata, CredentialLookupError> {
    ...
    match &metadata.cred_metadata {
        v1::CredentialMetadataV1::Identity(identity_credential_metadata) => {
        ...
        _ => {
            Err(CredentialLookupError::InvalidResponse("TODO (confirm) - currently v1 is just for identity credentials".to_string()))
        }
    }
}
...
pub async fn get_public_data_v1(
    ...
    let stream = presentation
        ...
        .map(|meta| {
            let mainnet_client = client.clone();
            async move { verify_credential_metadata_v1(mainnet_client, network, &meta, block).await }
        })
        .collect::<futures::stream::FuturesOrdered<_>>();
    stream.try_collect().await
}
