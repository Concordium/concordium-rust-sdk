            ...
            let summary =
                client
                    ...
                    .try_filter_map(|summary| async move {
                        ...
                        let target_address = info.execution_tree.address().known_or(
                            OnFinalizationError::Unknown(
                                "info.execution_tree.address()".to_string(),
                            ),
                        )?;

                        let entrypoint = info.execution_tree.entrypoint().known_or(
                            OnFinalizationError::Unknown(
                                "info.execution_tree.entrypoint()".to_string(),
                            ),
                        )?;

                        if target_address == self.target_address
                            && entrypoint == self.entrypoint.as_entrypoint_name()
                        {
                            Ok(Some(info))
                        } else {
                            Ok(None)
