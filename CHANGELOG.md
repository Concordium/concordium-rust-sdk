...
- Introduce basic types related to protocol level tokens (PLT)
  ...
  - `TokenAccountState`: The state of a protocol level token associated with some account.  
  - `TokenHolder`: A representation of the different token holder entities. Currently, only accounts are supported.
- Extend `UpdatePayload` with `CreatePlt` varsiant.
...
- Add the examples `plt-transfer.rs`, `plt-mint-and-burn.rs`, `plt-allow-and-deny-list`, `plt-token-client.rs`, and `plt-pause.rs` in the `examples` folder.
